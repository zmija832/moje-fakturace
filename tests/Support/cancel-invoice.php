...
[, $connectionName, $businessUuid, $invoiceUuid, $correlationUuid, $version, $barrier] = $argv + array_fill(0, 7, null);
...
$business->forceFill([
    'uuid' => (string) $businessUuid, 'display_name' => 'Souběžný test storna',
...
try {
    $app->make(InvoiceCancellationService::class)->cancel(
        (string) $invoiceUuid,
        (int) $version,
        (string) $correlationUuid,
        'Souběžné idempotentní storno.',
    );
    exit(0);
