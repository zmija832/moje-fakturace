...
<form method="POST" action="{{ $action }}" class="card space-y-5">@csrf @if($method !== 'POST') @method($method) @endif
    ...
    <div class="grid gap-5 sm:grid-cols-3">
        <div><label for="unit_price">Cena za MJ *</label><input id="unit_price" name="unit_price" inputmode="decimal" required value="{{ old('unit_price', $item->unit_price === null ? '' : \App\Domain\Invoices\InvoiceDecimal::formatInput($item->unit_price)) }}">@error('unit_price')<p class="field-error">{{ $message }}</p>@enderror</div>
