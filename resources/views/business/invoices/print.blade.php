...
<table><tr><td style="border:0"><h1>Faktura</h1><div class="muted">Vystavená faktura</div>@if($nonPayer)<div class="non-payer">Neplátce DPH</div>@endif</td><td class="right" style="border:0"><div class="document-number">{{ $document['document_number'] }}</div><div>{{ $document['currency'] }}</div></td></tr></table>
...
@if($nonPayer)
    ...
    @foreach($document['items'] as $item)<tr><td>{{ $item['description'] }}</td><td class="right">{{ $item['quantity'] }} {{ $item['unit'] }}</td><td class="right">{{ $item['unit_price'] }} {{ $document['currency'] }}</td><td class="right">{{ $item['discount'] }} {{ $document['currency'] }}</td><td class="right"><strong>{{ $item['total'] }} {{ $document['currency'] }}</strong></td></tr>@endforeach
    </tbody></table>
@else
...
<table class="summary">
    <tr><td>Mezisoučet</td><td class="right">{{ $document['totals']['subtotal_before_discount'] }} {{ $document['currency'] }}</td></tr>
    @if($document['totals']['has_discount'])<tr><td>Sleva celkem</td><td class="right">{{ $document['totals']['discount_total'] }} {{ $document['currency'] }}</td></tr>@endif
    @unless($nonPayer)<tr><td>Základ daně</td><td class="right">{{ $document['totals']['tax_base_total'] }} {{ $document['currency'] }}</td></tr><tr><td>DPH</td><td class="right">{{ $document['totals']['vat_total'] }} {{ $document['currency'] }}</td></tr>@endunless
    @if($document['totals']['has_rounding'])<tr><td>Zaokrouhlení</td><td class="right">{{ $document['totals']['rounding_adjustment'] }} {{ $document['currency'] }}</td></tr>@endif
    <tr><td class="total">Celkem</td><td class="right total">{{ $document['totals']['grand_total'] }} {{ $document['currency'] }}</td></tr>
</table>
