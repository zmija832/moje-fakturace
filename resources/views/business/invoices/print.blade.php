...
<body>
@php($nonPayer = ! $document['supplier']['is_vat_payer'])
...
<h2>Položky</h2>
<table><thead><tr><th>Popis</th><th class="right">Množství</th><th class="right">{{ $nonPayer ? 'Jednotková cena' : 'Cena bez DPH' }}</th><th class="right">Sleva</th><th class="right">{{ $nonPayer ? 'Částka' : 'Základ' }}</th><th>{{ $nonPayer ? 'Režim' : 'DPH / režim' }}</th><th class="right">Celkem</th></tr></thead><tbody>
@foreach($document['items'] as $item)<tr><td>{{ $item['description'] }}</td><td class="right">{{ $item['quantity'] }} {{ $item['unit'] }}</td><td class="right">{{ $item['unit_price'] }} {{ $document['currency'] }}</td><td class="right">{{ $item['discount'] }} {{ $document['currency'] }}</td><td class="right">{{ $item['tax_base'] }} {{ $document['currency'] }}</td><td>{{ $item['tax_label'] }}@unless($nonPayer)<br>{{ $item['vat_amount'] }} {{ $document['currency'] }}@endunless</td><td class="right">{{ $item['total'] }} {{ $document['currency'] }}</td></tr>@endforeach
</tbody></table>
<h2>{{ $nonPayer ? 'Souhrn položek' : 'Souhrn DPH' }}</h2><table><thead><tr><th>Režim</th><th class="right">{{ $nonPayer ? 'Částka' : 'Základ' }}</th><th class="right">DPH</th><th class="right">Celkem</th></tr></thead><tbody>@foreach($document['vat_summaries'] as $row)<tr><td>{{ $row['tax_label'] }}</td><td class="right">{{ $row['tax_base'] }} {{ $document['currency'] }}</td><td class="right">{{ $row['vat_amount'] }} {{ $document['currency'] }}</td><td class="right">{{ $row['total'] }} {{ $document['currency'] }}</td></tr>@endforeach</tbody></table>
<table class="summary"><tr><td>Mezisoučet před slevou</td><td class="right">{{ $document['totals']['subtotal_before_discount'] }} {{ $document['currency'] }}</td></tr><tr><td>Sleva celkem</td><td class="right">{{ $document['totals']['discount_total'] }} {{ $document['currency'] }}</td></tr><tr><td>{{ $nonPayer ? 'Částka po slevách' : 'Základ daně' }}</td><td class="right">{{ $document['totals']['tax_base_total'] }} {{ $document['currency'] }}</td></tr><tr><td>DPH</td><td class="right">{{ $document['totals']['vat_total'] }} {{ $document['currency'] }}</td></tr><tr><td>Celkem před zaokrouhlením</td><td class="right">{{ $document['totals']['total_before_rounding'] }} {{ $document['currency'] }}</td></tr><tr><td>Zaokrouhlení</td><td class="right">{{ $document['totals']['rounding_adjustment'] }} {{ $document['currency'] }}</td></tr><tr><td class="total">Celkem</td><td class="right total">{{ $document['totals']['grand_total'] }} {{ $document['currency'] }}</td></tr></table>
