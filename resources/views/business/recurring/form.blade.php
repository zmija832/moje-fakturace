...
<x-layouts.app :title="$editing ? 'Upravit opakovanou fakturu' : 'Nová opakovaná faktura'">
    ...
    <form class="card mt-6 space-y-5" method="POST" action="{{ $editing ? route('recurring.update', $template->uuid) : route('recurring.store') }}"
        ...
        <div class="grid gap-4 md:grid-cols-2">
            ...
            <label>Bankovní účet<select x-ref="bankAccount" class="input mt-1" name="bank_account_uuid"><option value="">Bez účtu</option>@foreach($options['bankAccounts'] as $account)<option value="{{ $account->uuid }}" x-show="currency === '{{ $account->currency }}'" :disabled="currency !== '{{ $account->currency }}'" @selected($selectedBankAccount === $account->uuid)>{{ $account->name }} · {{ \App\Domain\Invoices\InvoiceDecimal::currencyLabel($account->currency) }}</option>@endforeach</select></label>
