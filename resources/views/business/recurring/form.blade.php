...
<x-layouts.app :title="$editing ? 'Upravit opakovanou fakturu' : 'Nová opakovaná faktura'">
    ...
    <form class="card mt-6 space-y-5" method="POST" action="{{ $editing ? route('recurring.update', $template->uuid) : route('recurring.store') }}">
        ...
        <div class="grid gap-4 md:grid-cols-2">
            ...
            <label>Měna<select class="input mt-1" name="currency">@foreach($options['currencies'] as $code => $label)<option value="{{ $code }}" @selected(old('currency', $template->currency ?: 'CZK') === $code)>{{ $label }}</option>@endforeach</select></label>
            <label>Bankovní účet<select class="input mt-1" name="bank_account_uuid"><option value="">Bez účtu</option>@foreach($options['bankAccounts'] as $account)<option value="{{ $account->uuid }}" @selected(old('bank_account_uuid', $template->bank_account_uuid) === $account->uuid)>{{ $account->name }}</option>@endforeach</select></label>
            ...
            <label>Další vytvoření<input class="input mt-1" type="date" name="next_run_on" value="{{ old('next_run_on', $template->next_run_on?->format('Y-m-d') ?? today()->format('Y-m-d')) }}"></label>
