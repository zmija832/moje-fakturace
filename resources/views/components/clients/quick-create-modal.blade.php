    ...
    <div class="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-6 shadow-2xl">
        ...
        <form x-ref="quickClientForm" class="mt-6 space-y-5" @submit.prevent="createQuickClient" novalidate>
            <div class="grid gap-5 md:grid-cols-2">
                ...
                <div x-show="quickClientType === 'company'"><label for="quick-client-registration-number">IČO</label><input id="quick-client-registration-number" name="registration_number" maxlength="32"><p x-show="quickClientFieldError('registration_number')" x-text="quickClientFieldError('registration_number')" class="field-error"></p></div>
            ...
            <div class="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end"><button type="button" class="button-secondary" @click="closeQuickClient">Zrušit</button><button type="submit" class="button-primary" :disabled="quickClientSubmitting" x-text="quickClientSubmitting ? 'Ukládám…' : 'Vytvořit a vybrat'"></button></div>
        </form>
    </div>
</div>
