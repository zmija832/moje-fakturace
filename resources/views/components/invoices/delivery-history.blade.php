...
<section id="invoice-delivery-history" class="card mt-6 scroll-mt-6">
    <div>
        ...
        <p class="text-sm text-slate-600">Historie skutečných pokusů; záznamy se nemažou.</p>
    </div>
        ...
        <div class="mt-4 space-y-3">
            @foreach($invoice->emailDeliveries as $delivery)
                <article class="rounded-lg border border-slate-200 p-3 text-sm">
                    <div class="flex justify-between gap-3"><strong>{{ $delivery->recipient_email }}</strong><span>{{ $delivery->status->label() }}</span></div>
                    <div class="mt-1">{{ $delivery->subject }}</div>
                    <div class="mt-1 text-xs text-slate-500">Pokus {{ $delivery->attempted_at->format('j. n. Y H:i:s') }}@if($delivery->sent_at) · úspěch {{ $delivery->sent_at->format('j. n. Y H:i:s') }}@endif @if($delivery->failed_at) · chyba {{ $delivery->failed_at->format('j. n. Y H:i:s') }}@endif</div>
