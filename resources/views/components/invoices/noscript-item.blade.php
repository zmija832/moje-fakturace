...
<noscript><fieldset class="mt-5 rounded-xl border border-slate-200 p-4"><legend class="px-2 font-semibold">Položka 1</legend><input type="hidden" name="items[0][position]" value="1"><div class="grid gap-4 md:grid-cols-2">
    ...
    <div><label for="ns-vat">Sazba DPH *</label><select id="ns-vat" name="items[0][vat_rate_uuid]" required>@foreach($vatRates as $rate)<option value="{{ $rate->uuid }}" @selected(old('items.0.vat_rate_uuid')===$rate->uuid)>{{ $rate->name }}</option>@endforeach</select></div>
