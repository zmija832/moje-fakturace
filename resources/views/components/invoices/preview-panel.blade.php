...
@if($isVatPayer)
    <section class="rounded-xl border border-slate-200 bg-slate-50 p-4" aria-live="polite">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <button class="button-secondary" type="button" x-show="previewError" @click="refreshPreview()" :disabled="loading">Zkusit přepočítat</button>
        </div>
...
@else
    <div class="flex justify-end" aria-live="polite" x-show="previewError">
        <div class="rounded-lg border border-red-200 bg-red-50 p-3 text-sm text-red-800"><span x-text="previewError"></span><button class="ml-3 font-semibold underline" type="button" @click="refreshPreview()" :disabled="loading">Zkusit znovu</button></div>
    </div>
@endif
