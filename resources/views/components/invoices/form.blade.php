...
@php
    $values = $revision ? ['customer_uuid'=>$revision->customerSnapshot->source_client_uuid,'bank_account_uuid'=>$revision->bankAccountSnapshot?->source_bank_account_uuid,'currency'=>$revision->currency,'issued_on'=>$revision->issued_on->format('Y-m-d'),'taxable_supply_on'=>$revision->taxable_supply_on->format('Y-m-d'),'due_on'=>$revision->due_on->format('Y-m-d'),'payment_method'=>$revision->payment_method->value,'variable_symbol'=>$revision->variable_symbol,'note'=>$revision->note,'invoice_discount_type'=>$revision->invoice_discount_type->value,'invoice_discount_value'=>\App\Domain\Invoices\InvoiceDecimal::formatInput($revision->invoice_discount_value)] : $defaults;
    $storedItems = $revision?->items->map(fn($item)=>['description'=>$item->description,'quantity'=>\App\Domain\Invoices\InvoiceDecimal::formatInput($item->quantity),'unit'=>$item->unit,'unit_price'=>\App\Domain\Invoices\InvoiceDecimal::formatInput($item->unit_price),'discount_type'=>$item->discount_type->value,'discount_value'=>$item->discount_value === null ? null : \App\Domain\Invoices\InvoiceDecimal::formatInput($item->discount_value),...$isVatPayer ? ['vat_rate_uuid'=>$item->vatSnapshot->source_vat_rate_uuid] : []])->all();
    ...
    $editorConfig = ['items'=>$initialItems,'errors'=>$errors->toArray(),'previewUrl'=>route('invoices.preview'),'catalogSearchUrl'=>route('invoice-catalog.search'),'clientStoreUrl'=>$canCreateClientInline ? route('clients.store') : null,'aresLookupUrl'=>$canCreateClientInline ? route('clients.ares.lookup') : null,'csrf'=>csrf_token(),'isVatPayer'=>$isVatPayer,'defaultVatRateUuid'=>$isVatPayer ? $defaultVatRateUuid : null,'defaultBankAccounts'=>$defaultBankAccounts,'currency'=>old('currency',$values['currency'] ?? 'CZK'),'paymentMethod'=>old('payment_method',$values['payment_method'] ?? 'bank_transfer')];
    ...
    $itemErrorLabels = ['position'=>'Pořadí','description'=>'Popis','quantity'=>'Množství','unit'=>'Jednotka','unit_price'=>$isVatPayer ? 'Cena bez DPH' : 'Cena za MJ','discount_type'=>'Sleva','discount_value'=>'Hodnota slevy','vat_rate_uuid'=>'Sazba DPH'];
...
<form id="invoice-form" x-ref="form" method="POST" action="{{ $action }}" class="space-y-6" @input="queuePreview" @change="queuePreview">
    ...
    <section class="card"><h2 class="mb-5 text-lg font-bold">1. Odběratel a platba</h2><div class="grid gap-5 md:grid-cols-2">
        ...
        <div><label for="currency">Měna *</label><select id="currency" name="currency" x-model="currency" @change="applyDefaultBankAccount" required @error('currency') aria-invalid="true" aria-describedby="currency-error" @enderror>@foreach($currencies as $value=>$label)<option value="{{ $value }}" @selected(old('currency',$values['currency'] ?? 'CZK') === $value)>{{ $label }}</option>@endforeach</select>@error('currency')<p id="currency-error" class="field-error">{{ $message }}</p>@enderror</div>
        ...
        <div x-show="paymentMethod === 'bank_transfer'"><label for="bank_account_uuid">Bankovní účet *</label><select x-ref="bankAccountSelect" id="bank_account_uuid" name="bank_account_uuid" @error('bank_account_uuid') aria-invalid="true" aria-describedby="bank_account_uuid-error" @enderror><option value="">Vyberte účet</option>@foreach($bankAccounts as $account)<option value="{{ $account->uuid }}" data-currency="{{ $account->currency }}" x-show="currency === '{{ $account->currency }}'" :disabled="currency !== '{{ $account->currency }}'" @selected(old('bank_account_uuid',$values['bank_account_uuid'] ?? ($defaultBankAccounts[old('currency',$values['currency'] ?? 'CZK')] ?? '')) === $account->uuid)>{{ $account->name }} · {{ \App\Domain\Invoices\InvoiceDecimal::currencyLabel($account->currency) }}</option>@endforeach</select>@error('bank_account_uuid')<p id="bank_account_uuid-error" class="field-error">{{ $message }}</p>@enderror</div>
    </div></section>
    <x-invoices.header-fields :values="$values" :discount-types="$discountTypes" :is-vat-payer="$isVatPayer" />
