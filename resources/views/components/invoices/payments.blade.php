...
<section id="invoice-payments" class="card mt-6 scroll-mt-6" aria-labelledby="invoice-payments-heading">
    ...
    <dl class="mt-5 grid gap-4 sm:grid-cols-3">
        <div class="rounded-xl bg-slate-50 p-4"><dt class="text-sm text-slate-500">Celkem</dt><dd class="mt-1 text-lg font-bold">{{ \App\Domain\Invoices\InvoiceDecimal::formatMoney($summary->grandTotal, $invoice->currency) }}</dd></div>
        <div class="rounded-xl bg-emerald-50 p-4"><dt class="text-sm text-emerald-700">Uhrazeno</dt><dd class="mt-1 text-lg font-bold text-emerald-900">{{ \App\Domain\Invoices\InvoiceDecimal::formatMoney($summary->paidTotal, $invoice->currency) }}</dd></div>
        <div class="rounded-xl bg-amber-50 p-4"><dt class="text-sm text-amber-700">Zbývá uhradit</dt><dd class="mt-1 text-lg font-bold text-amber-900">{{ \App\Domain\Invoices\InvoiceDecimal::formatMoney($hasOutstanding ? $summary->remainingTotal : '0', $invoice->currency) }}</dd></div>
    </dl>
    ...
    @if(\App\Domain\Invoices\InvoiceDecimal::compare($summary->overpaymentTotal, '0') > 0)<p class="mt-4 text-sm font-semibold text-violet-700">Historický přeplatek: {{ \App\Domain\Invoices\InvoiceDecimal::formatMoney($summary->overpaymentTotal, $invoice->currency) }}</p>@endif
    ...
    @if($canManage && $hasOutstanding)
        <form id="invoice-payment-entry" method="POST" action="{{ route('invoices.payments.store', $invoice->uuid) }}" class="mt-6 scroll-mt-6 border-t border-slate-200 pt-6">
            ...
            <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><div><h3 class="font-semibold">Zaznamenat úhradu</h3><p class="text-sm text-slate-600">Výchozí částka odpovídá plné zbývající úhradě; lze zadat i nižší částečnou platbu.</p></div><button class="button-secondary" type="button" onclick="document.getElementById('payment-amount').value='{{ \App\Domain\Invoices\InvoiceDecimal::formatInput($summary->remainingTotal) }}'">Plná úhrada</button></div>
            <div class="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                <div><label for="payment-amount">Částka</label><input id="payment-amount" name="amount" value="{{ old('amount', \App\Domain\Invoices\InvoiceDecimal::formatInput($summary->remainingTotal)) }}" inputmode="decimal" required>@error('amount')<p class="field-error">{{ $message }}</p>@enderror</div>
