...
<form method="GET" action="{{ route('invoices.index') }}" class="card mb-6">
    <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <div><label for="visibility">Zobrazení</label><select id="visibility" name="visibility"><option value="active" @selected(($filters['visibility'] ?? 'active') === 'active')>Aktivní</option><option value="archived" @selected(($filters['visibility'] ?? '') === 'archived')>Archivované</option><option value="all" @selected(($filters['visibility'] ?? '') === 'all')>Vše</option></select></div>
        <div><label for="status">Typ stavu</label><select id="status" name="status"><option value="all">Všechny</option><option value="draft" @selected(($filters['status'] ?? '') === 'draft')>Koncepty</option><option value="issued" @selected(($filters['status'] ?? '') === 'issued')>Vystavené</option></select></div>
