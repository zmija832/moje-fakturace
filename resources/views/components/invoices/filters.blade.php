...
<form method="GET" action="{{ route('invoices.index') }}" class="card mb-6">
    <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <div><label for="status">Stav</label><select id="status" name="status"><option value="all">Všechny</option><option value="draft" @selected(($filters['status'] ?? '') === 'draft')>Návrhy</option><option value="issued" @selected(($filters['status'] ?? '') === 'issued')>Vystavené</option></select></div>
        ...
        <div class="flex items-center gap-2 pt-6"><input id="overdue" name="overdue" type="checkbox" value="1" @checked($filters['overdue'] ?? false)><label for="overdue">Informativně po splatnosti</label></div>
    </div>
