        ...
        <article class="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
            <header class="flex flex-col gap-5 border-b border-slate-200 p-6 sm:flex-row sm:items-start sm:justify-between">
                ...
                <div class="sm:text-right">
                    ...
                    <p class="text-3xl font-bold tabular-nums">{{ $document['totals']['grand_total'] }} {{ $document['currency_label'] }}</p>
                    ...
                    <table class="min-w-full text-sm">
                        ...
                        <tbody class="divide-y divide-slate-200">
                            @foreach($document['items'] as $item)
                                <tr><td class="py-3 pr-4">{{ $item['description'] }}</td><td class="py-3 pr-4 text-right">{{ $item['quantity'] }} {{ $item['unit'] }}</td><td class="py-3 pr-4 text-right">{{ $item['unit_price'] }} {{ $document['currency_label'] }}</td><td class="py-3 pr-4 text-right">{{ $item['discount'] }} {{ $document['currency_label'] }}</td>@unless($document['is_non_payer'])<td class="py-3 pr-4">{{ $item['tax_label'] }} · {{ $item['vat_amount'] }} {{ $document['currency_label'] }}</td>@endunless<td class="py-3 text-right font-bold">{{ $item['total'] }} {{ $document['currency_label'] }}</td></tr>
                            @endforeach
                        </tbody>
                    </table>
                </div>
            ...
            @unless($document['is_non_payer'])
                <section class="border-t border-slate-200 p-6"><h2 class="font-bold">Souhrn DPH</h2><div class="mt-3 space-y-2 text-sm">@foreach($document['vat_summaries'] as $summary)<div class="flex flex-wrap justify-between gap-3"><span>{{ $summary['tax_label'] }}</span><span>Základ {{ $summary['tax_base'] }} · DPH {{ $summary['vat_amount'] }} · Celkem {{ $summary['total'] }} {{ $document['currency_label'] }}</span></div>@endforeach</div></section>
            @endunless
            ...
            <footer class="grid gap-6 border-t border-slate-200 bg-slate-50 p-6 md:grid-cols-2">
                <div class="text-sm">@if($document['bank_account'])<p><strong>Bankovní účet:</strong> {{ $document['bank_account']['domestic'] }}</p>@if($document['bank_account']['iban'])<p>IBAN {{ $document['bank_account']['iban'] }}</p>@endif @endif @if($document['note'])<p class="mt-3 whitespace-pre-line">{{ $document['note'] }}</p>@endif @if($document['outro'])<div class="mt-4 border-t border-slate-200 pt-4"><p class="font-semibold">Doplňující informace</p><p class="mt-1 whitespace-pre-line text-slate-600">{{ $document['outro'] }}</p></div>@endif</div>
                <dl class="space-y-2 text-sm md:ml-auto md:min-w-72">@if($document['totals']['has_discount'])<div class="flex justify-between gap-4"><dt>Sleva</dt><dd>{{ $document['totals']['discount_total'] }} {{ $document['currency_label'] }}</dd></div>@endif @unless($document['is_non_payer'])<div class="flex justify-between gap-4"><dt>DPH</dt><dd>{{ $document['totals']['vat_total'] }} {{ $document['currency_label'] }}</dd></div>@endunless @if($document['totals']['has_rounding'])<div class="flex justify-between gap-4"><dt>Zaokrouhlení</dt><dd>{{ $document['totals']['rounding_adjustment'] }} {{ $document['currency_label'] }}</dd></div>@endif<div class="flex justify-between gap-5 border-t border-slate-300 pt-3 text-xl font-bold"><dt>Celkem</dt><dd>{{ $document['totals']['grand_total'] }} {{ $document['currency_label'] }}</dd></div></dl>
            </footer>
        </article>
