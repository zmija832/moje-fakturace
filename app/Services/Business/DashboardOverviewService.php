...
{
    public function __construct(private readonly BusinessConnectionResolver $resolver) {}
...
SELECT i.currency,
 ...
 SUM(CASE WHEN i.due_on < CURRENT_DATE AND r.grand_total > COALESCE(p.paid_total,0) THEN r.grand_total-COALESCE(p.paid_total,0) ELSE 0 END) overdue_total,
 SUM(CASE WHEN i.due_on < CURRENT_DATE AND r.grand_total > COALESCE(p.paid_total,0) THEN 1 ELSE 0 END) overdue_count,
...
SQL));
        ...
        return ['currencies' => $currencies, 'issuedThisMonth' => $issued, 'paidThisMonth' => $paid, 'draftCount' => Invoice::query()->where('status', 'draft')->whereNull('archived_at')->count(), 'recurringActive' => RecurringInvoiceTemplate::query()->where('is_active', true)->count(), 'recurringNext' => RecurringInvoiceTemplate::query()->where('is_active', true)->min('next_run_on'), 'dueRecurring' => RecurringInvoiceTemplate::query()->where('is_active', true)->whereDate('next_run_on', '<=', today())->count(), 'failedAutomation' => RecurringInvoiceRun::query()->where('status', 'failed')->count() + InvoiceReminder::query()->where('status', 'failed')->count() + InvoicePaidNotification::query()->where('status', 'failed')->count(), 'adminPaidAlerts' => InvoicePaidNotification::query()->where('recipient_type', 'admin')->where('status', 'internal')->count()];
    }
}
