...
class RunInvoiceAutomationCommand extends Command
{
    ...
    protected $description = 'Idempotentně synchronizuje bankovní platby a zpracuje opakované faktury, upomínky i retry notifikací po zaplacení';
    ...
    public function handle(ActiveBusinessContext $context, BusinessDate $businessDate): int
    {
        ...
        foreach ($businesses as $business) {
            ...
            try {
                ...
                try {
                    $bankSync = app(FioBankSyncService::class)->syncAll($today);
                } catch (Throwable $exception) {
                    report($exception);
                    $bankSync = ['accounts' => [], 'failed' => 1];
                }
                ...
                $this->line("{$business->display_name}: bank sync ".count($bankSync['accounts'])."/{$bankSync['failed']}, recurring {$recurring['processed']}/{$recurring['failed']}, reminders {$reminders['processed']}/{$reminders['failed']}, paid notifications {$paidNotifications['processed']}/{$paidNotifications['failed']}");
                $failed = $failed || $bankSync['failed'] > 0 || $recurring['failed'] > 0 || $reminders['failed'] > 0 || $paidNotifications['failed'] > 0;
            } catch (Throwable $e) {
                $failed = true;
                report($e);
